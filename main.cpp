...
// Num of snake body to increment
const int NUM_SNAKE_BODY_EXTEND = 4;
...
const unsigned int NUM_FOOD = 4;
int FOOD_X[NUM_FOOD] = { 0 };
int FOOD_Y[NUM_FOOD] = { 0 };
...
bool snake_ate_food() {
    for (int i(0); i < snake.size(); i++) {
        ...
        {
            if (snake[i].x == FOOD_X[j] && snake[i].y == FOOD_Y[j]) {
                return true;
            }
        }
    }
...
void generate_food_coord() {
    ...
    for (int i = 0; i < NUM_FOOD; i++)
    {
        FOOD_X[i] = 3 + rand() % (MAP_WIDTH - 3);
        FOOD_Y[i] = 3 + rand() % (MAP_HEIGHT - 3);
    }
}
...
int main () {
    ...
    input_on();
    memset(&FOOD_X, 0, sizeof(int)*NUM_FOOD);
    memset(&FOOD_Y, 0, sizeof(int)*NUM_FOOD);
    for (int i = 0; i < NUM_FOOD; i++)
    {
        generate_food_coord();
    }
    ...
    while (true) {
        ...
        if (snake_ate_food()) {
            ...
            // Here snake body is extended when fruit is eaten
            for (int i=0; i < NUM_SNAKE_BODY_EXTEND; ++i)
            {
